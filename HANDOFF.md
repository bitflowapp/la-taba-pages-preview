# HANDOFF — TABA2, RC de piloto integrada

Este archivo es la resolución del único conflicto de contenido de la integración:
**las dos ramas fuente entregaron su propio `HANDOFF.md` en la misma ruta**. Los dos
documentan trabajo disjunto —una el circuito de pedidos real, la otra el storefront
comercial— así que se conservan **completos y sin editar**. Quedarse con uno habría
borrado la entrega del otro, que es exactamente lo que un `ours`/`theirs` a ciegas hace.

El estado de la integración en sí —HEADs, conflictos, migraciones, deploy, combos,
precios pendientes, compra E2E y riesgos— está en `INTEGRATION-HANDOFF.md`.

---

# Parte A · Circuito de pedidos real (`feature/taba2-real-orders-ops`)
...
# Parte B · Storefront comercial (`feature/taba2-commercial-storefront`)
