...
/*
 * El orden en que se dibujan los destinos. Es la lista que la persona VE, y
 * tiene que contener a todas las superficies que existen: una superficie que no
 * esta aca no tiene ningun boton que lleve a ella, aunque se pueda abrir, aunque
 * tenga permiso y aunque su RPC funcione.
 *
 * Paso: `team-access` y `operations-config` se agregaron a
 * BUSINESS_OPERATION_VIEWS -con su etiqueta, su permiso y su despacho- y nadie
 * las agrego aca. Quedaron invisibles en produccion. Las pruebas no lo vieron
 * porque llaman a las funciones de render directamente, sin pasar por la
 * navegacion. `tests/panel-navigation-completa.test.mjs` ata las dos listas para
 * que la proxima superficie nazca alcanzable.
 *
 * `team-access` va temprano a proposito: es el unico destino donde hay una
...
/*
 * `product-create` decia «Alta». En un Panel que ahora tambien da de alta
 * PERSONAS -«Solicitudes de acceso»- ese boton decia lo que no es: se lee como
 * alta de alguien del equipo y abre el formulario de un producto. Dice lo que
 * hace.
 */
