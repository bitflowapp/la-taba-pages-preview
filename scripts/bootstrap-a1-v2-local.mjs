...
const c=process.argv[2],platformContainer=process.argv[3];
if(process.env.TABA_LOCAL_PAYMENT_DB!=='1'||!/^taba-a1-a4-local-[\w-]+$/.test(c||'') ||
  !/^supabase_db_[\w-]+$/.test(platformContainer||''))throw Error('Explicit disposable DB and local platform schema source required');
...
sql(`create role anon; create role authenticated; create role service_role bypassrls;
 ...
 create schema vault;`);
// Only schema definitions, never data, from the already-running LOCAL platform.
const platform=execFileSync('docker',['exec',platformContainer,'pg_dump','-U','postgres','-d','postgres','--schema-only','--schema=auth','--no-owner','--no-privileges'],{encoding:'utf8'});
// Application-specific auth triggers belong to the source project, not this
// synthetic cluster. Auth login and Storage service behavior are not under test.
sql(platform.replace(/^CREATE TRIGGER[^;]+;/gm,''));
