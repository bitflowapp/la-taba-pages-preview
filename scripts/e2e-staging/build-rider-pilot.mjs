...
const built=spawnSync('cmd.exe',['/d','/c','gradlew.bat',':app:assembleRelease','--console=plain'],{
...
const versionName=aapt.stdout.match(/versionName='([^']+)'/)?.[1];
if(aapt.status!==0||packageId!=='com.lataba.rider.pilot'||versionName!=='0.1.0-canonical-pilot')
 throw Error('PILOT_PACKAGE_IDENTITY_MISMATCH');
console.log(JSON.stringify({signed:true,packageId,versionName,certificateSha256:signer,
