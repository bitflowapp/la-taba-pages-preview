...
export async function instalarDatosDePrueba(page, { conSesion = true } = {}) {
  await page.route(`${SUPABASE_URL}/**`, async (route) => {
    ...
    // La bandeja de solicitudes. Los datos son los de la solicitud REAL que
    // esta esperando en produccion -mismos nombres de campo que devuelve
    // identity_list_access_requests- para que la captura muestre exactamente lo
    // que el duenio va a ver en su telefono. Nada de esto toca produccion: el
    // interceptor contesta local.
    if (p.includes('/rpc/identity_list_access_requests')) {
      const pedido = (() => {
        try { return JSON.parse(route.request().postData() || '{}'); } catch { return {}; }
      })();
      const estado = pedido.p_status || 'pending';
      return json(route, estado === 'pending' ? solicitudesDeAcceso() : []);
    }
...
/**
 * La solicitud de acceso que espera en produccion, con la forma exacta que
...
export function solicitudesDeAcceso() {
  return [{
    ...
    user_id: 'be333e7e-2c18-4674-af7f-97f581646550',
    email: 'jariel1970+rider@gmail.com',
    full_name: 'Marco Luna',
    contact_phone: '2996209136',
    requested_access: 'rider',
    status: 'pending',
    attempt_count: 1,
    requested_at: '2026-08-17T22:22:05.681Z',
    decided_at: null,
    granted_role: null,
