-- ============================================================================
--  Checkout Pro: la dirección viaja con su punto hasta el pedido
-- ============================================================================
--
--  MEDIDO SOBRE UN PEDIDO REAL
--  ---------------------------
--  LT-0097, comprado desde un navegador contra el sitio publicado con la
--  ubicación confirmada en el Perfil. La dirección guardada quedó correcta:
--
--    customer_addresses: latitude -38.9539, longitude -68.0596,
--                        geolocation_accuracy 12, source 'gps'
--
--  y el pedido que creó Checkout Pro quedó así:
--
--    orders: delivery_address_source 'gps',
--            delivery_latitude NULL, delivery_longitude NULL
--
--  El pedido afirmaba origen GPS sin tener un solo punto. Dos huecos en fila:
--
--    1. `create_checkout_session` ACEPTA 'latitude' y 'longitude' entre las
--       claves permitidas de la dirección, las valida por longitud junto al
--       resto... y después arma `address_snapshot` sin ellas. Con dirección
--       guardada tampoco copia las de `customer_addresses`. Se pierden ahí.
--    2. `finalize_paid_checkout_session` proyecta el snapshot al pedido y
--       escribe `delivery_address_source`, pero no tiene de dónde sacar el
--       punto porque el snapshot ya no lo trae.
--
--  Consecuencia aguas abajo: `rider_map_capture_order_location_snapshot`
--  fotografía la ubicación del pedido al crearlo, y la instantánea es
--  inmutable. Un pedido de Checkout Pro nacía sin punto de entrega para
--  siempre, y la app del Rider mostraba «No hay coordenadas autorizadas» sin
--  que hubiera nada roto en el mapa: el dato nunca había llegado.
--
--  Los demás medios de pago no comparten el defecto: escriben el pedido
--  directamente con `delivery_latitude` / `delivery_longitude`. Era propio del
--  camino de Checkout Pro.
--
--  QUÉ CAMBIA
--  ----------
--    - el snapshot del checkout lleva latitude, longitude y geolocation_accuracy;
--    - con dirección guardada salen de `customer_addresses`, que es la autoridad;
--    - escritas a mano, se validan como en `upsert_current_customer_address`;
--    - la finalización las proyecta a las columnas `delivery_*` del pedido;
--    - 'geolocation_accuracy' se suma a las claves permitidas de la dirección.
--
--  COHERENCIA DEL ORIGEN
--  ---------------------
--  Sin coordenadas, un origen 'gps' o 'geocoder' se degrada a 'manual' en vez
--  de abortar: la dirección postal sigue siendo válida y el pedido tiene que
--  poder avanzar. Lo que ya no puede es viajar afirmando un origen que no
--  tiene respaldo, que es exactamente lo que mostraba LT-0097.
--
--  Esto además cierra un choque latente: `rider_map_order_location_snapshots`
--  restringe `customer_source` a manual/gps/geocoder/previous_order/qa_fixture.
--  El `coalesce(..., 'checkout_session')` de la finalización sólo actúa cuando
--  el snapshot no trae origen -imposible en delivery, porque `create_checkout_session`
--  ya lo valida-, así que con coordenadas presentes el valor proyectado siempre
--  pertenece al conjunto permitido y el trigger no puede abortar el alta.
--
--  PEDIDOS ANTERIORES
--  ------------------
--  No se tocan. La instantánea del mapa se toma en el alta y es inmutable por
--  contrato; rellenarla después sería inventar dónde estuvo alguien. LT-0097
--  se queda sin punto de cliente y así se documenta.
-- ============================================================================
...
begin
  ...
  -- avanzar; lo que no puede es viajar diciendo `gps` sin coordenadas.
