-- ============================================================================
--  Checkout Pro: la sesión no nace, y por lo tanto el stock no se reserva, sin
--  un punto de entrega confirmado
-- ============================================================================
--
--  Complemento de 20260808190000. Aquel impone el contrato en las direcciones,
--  en el pedido y en el camino de pedido directo. Este cierra el camino de pago
--  online, que es el que usó el primer pedido humano.
--
--  DÓNDE SE RECHAZA
--  ----------------
--  En `create_checkout_session`, ANTES de insertar la sesión y antes del bucle
--  que bloquea productos y descuenta stock. Si el punto falta, no hay sesión, no
--  hay reserva y no hay intención de pago: el carrito queda intacto y el cliente
--  vuelve a confirmar la ubicación. Validar después de reservar dejaría stock
--  retenido por un pedido que nunca podía existir.
--
--  EN LA FINALIZACIÓN NO SE ABORTA
--  -------------------------------
--  `finalize_paid_checkout_session` corre DESPUÉS de que el dinero se movió.
--  Ahí un `raise` sería lo peor posible: pago tomado y transacción revertida en
--  loop. Si una sesión delivery llegara sin punto —imposible por lo anterior, y
--  por eso mismo digno de mirar— se deriva a revisión manual con el mismo
--  mecanismo que ya existe para una reserva vencida: la sesión y la intención de
--  pago quedan marcadas, el pedido no se crea y una persona decide.
--
--  El snapshot de la sesión transporta `location_source` y `location_confirmed_at`
--  junto a las coordenadas, así que el pedido nace con la misma confirmación que
--  vio el cliente, sin volver a consultar la dirección guardada —que para
--  entonces pudo cambiar o borrarse.
-- ============================================================================
