...
security definer
set search_path = pg_catalog, public, extensions, pg_temp
as $request_fiscal_document_rg5616$
declare
...
begin
  ...
  -- RG 5616: una factura nueva no se encola sin la condición del receptor.
  ...
  values(v_document.id,'queued','operator',auth.uid(),jsonb_build_object('source_type',p_source_type,'policy_version',v_policy.policy_version,'recipient_vat_condition_id',v_policy.recipient_vat_condition_id));
  return jsonb_build_object('fiscal_document_id',v_document.id,'state','queued','idempotent_replay',false);
end;
$request_fiscal_document_rg5616$;
...
begin
  ...
  if tg_op='UPDATE' and old.state in ('authorized','credited') and (
    ...
    or new.recipient_vat_condition_id is distinct from old.recipient_vat_condition_id
  ) then raise exception 'los datos autorizados son inmutables' using errcode='55000'; end if;
  return case when tg_op='DELETE' then old else new end;
end;
$protect_fiscal_rg5616$;
