...
function order(overrides = {}) {
  ...
  return {
    ...
    items: overrides.items || [{ productId: 'p-muzzarella', name: 'Vacio snapshot', quantity: 1, unitPrice: snapshotSubtotal }],
...
test('business report: total inconsistente se recalcula desde snapshots del pedido', () => {
  const report = buildBusinessReport([
    order({
      items: [{ productId: 'p-muzzarella', name: 'Vacio snapshot', quantity: 2, unitPrice: 1000 }],
      subtotal: 2000,
      discountTotal: 200,
      deliveryFee: 300,
      total: 9999,
