...
import { openQaWindow } from '../../scripts/controlled-production/qa-window.mjs';
...
test('CP: cliente → pago manual → panel → rider → GPS → código → entregado', async ({ browser, browserName }, testInfo) => {
  ...
  // Closed outside QA runs; closed again below whatever happens.
  const qaWindow = await openQaWindow(owner.c, BUSINESS);
  try {
