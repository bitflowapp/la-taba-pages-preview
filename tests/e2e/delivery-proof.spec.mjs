...
test('Delivery proof photo: rider adjunta foto y negocio ve comprobante local/demo', async ({ browser }) => {
  ...
  await page.locator('[data-view="business"] [data-open-pin]').click();
  ...
  await page.goto('/#business');
  await expect(page.locator('[data-view="business"]')).toBeVisible();
  await page.locator('[data-order-filter="done"]').click();
  const proof = page.locator('[data-delivery-proof-summary="LT-0002"]');
  await expect(proof).toBeVisible();
  await expect(proof).toContainText('Foto de entrega');
  await expect(proof.locator('[data-delivery-proof-thumb]')).toBeVisible();
  await expect(page.locator('[data-order-inbox]')).toContainText('Entregado');

  await proof.locator('[data-order-proof="LT-0002"]').click();
  await expect(page.locator('[data-delivery-proof-modal]')).toBeVisible();
  await expect(page.locator('[data-delivery-proof-modal]')).toContainText('Comprobante tomado');
  await expect(page.locator('[data-delivery-proof-modal-image]')).toBeVisible();
