...
test('service worker caches only existing GitHub Pages assets', () => {
  ...
  assert.equal(cacheNameMatch[1], 'la-taba-runtime-v53-confirmar-antes-de-escribir');
  ...
  assert.ok(assets.includes('./styles.css?v=44'));
  assert.ok(assets.includes('./styles/storefront.css?v=44'));
  assert.ok(assets.includes('./styles/responsive.css?v=44'));
  assert.ok(assets.includes('./styles/showcase.css?v=44'));
