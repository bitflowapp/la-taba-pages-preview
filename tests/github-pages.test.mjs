...
test('service worker caches only existing GitHub Pages assets', () => {
  ...
  assert.equal(cacheNameMatch[1], 'la-taba-runtime-v92-el-panel-abre-sin-red-y-la-bandeja-no-se-rearma');
  ...
  assert.ok(assets.includes('./styles.css?v=56'));
  assert.ok(assets.includes('./styles/storefront.css?v=56'));
  assert.ok(assets.includes('./styles/responsive.css?v=56'));
  assert.ok(assets.includes('./styles/showcase.css?v=56'));
  assert.ok(assets.includes('./js/app.js?v=48'));
