...
test('service worker caches only existing GitHub Pages assets', () => {
  ...
  assert.equal(cacheNameMatch[1], 'la-taba-runtime-v47-first-physical-e2e');
  ...
  assert.ok(assets.includes('./styles.css?v=43'));
  assert.ok(assets.includes('./styles/storefront.css?v=43'));
  assert.ok(assets.includes('./styles/responsive.css?v=43'));
  assert.ok(assets.includes('./styles/showcase.css?v=43'));
